...
int stopBabyLooper( TChain* chain, TString output_file="histos.root", bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  unsigned int nEvents_lowDM=0;
  double nEvents_lowDM_wgt=0;
  unsigned int nEvents_highDM=0;
  double nEvents_highDM_wgt=0;
  unsigned int nEvents_pass_tauVeto_lowDM = 0;
  double nEvents_pass_tauVeto_lowDM_wgt = 0.0;
  unsigned int nEvents_pass_tauVeto_highDM = 0;
  double nEvents_pass_tauVeto_highDM_wgt = 0.0;
  ...
  TH1D *h_genHadTau_pt = new TH1D("h_genHadTau_pt", "gen hadronic tau, pT", 40, 0.0, 200.0);
  TH1D *h_genHadTau_pt_isIsoTrack = new TH1D("h_genHadTau_pt_isIsoTrack", "gen hadronic tau, pT, matched to isoTrack", 40, 0.0, 200.0);
  TH1D *h_genHadTau_pt_isTauId = new TH1D("h_genHadTau_pt_isTauId", "gen hadronic tau, pT, matched to reco tau", 40, 0.0, 200.0);
  TH1D *h_genHadTau_pt_isIsoTrackOrTauId = new TH1D("h_genHadTau_pt_isIsoTrackOrTauId", "gen hadronic tau, pT, matched to isoTrack OR reco tau", 40, 0.0, 200.0);
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if( !is_data() && isTTbar && is_2lep ){
	  ...
	  if( pfmet()>=450.0 ){
	    ...
	    //diNuSF_err = 0.169365; // from binned
	...
	if(isNominal){
	  if(MT2W()<200.0 && nSelJets>=3){
	    nEvents_lowDM++;
	    nEvents_lowDM_wgt+=wgt;
	  }
	  if(MT2W()>=200.0 && nSelJets>=3){
	    nEvents_highDM++;
	    nEvents_highDM_wgt+=wgt;
	  }
	}

	// Check if this is a tau event
	double minDr_tau=0.4;
	if(isNominal){

	  for(int iGen=0; iGen<(int)genleps_p4().size(); iGen++){
	    if( abs(genleps_id().at(iGen))==15 &&
		genleps_fromHardProcessDecayed().at(iGen) &&
		(genleps_gentaudecay().at(iGen)==3 || genleps_gentaudecay().at(iGen)==4) &&
		genleps_isLastCopy().at(iGen)                ){
	    
	      h_genHadTau_pt->Fill( genleps_p4().at(iGen).Pt(), wgt );

	      bool isMatchedToVetoTau=false;
	      for(int iTau=0; iTau<(int)tau_p4().size(); iTau++){
		if( ROOT::Math::VectorUtil::DeltaR( tau_p4().at(iTau), genleps_p4().at(iGen) ) < minDr_tau &&
		    tau_isVetoTau().at(iTau) ){
		  h_genHadTau_pt_isTauId->Fill( genleps_p4().at(iGen).Pt(), wgt );
		  isMatchedToVetoTau=true;
		  break;
		}
	      }

	      bool isMatchedToIsoTrack=false;
	      for(int iTrk=0; iTrk<(int)isoTracks_p4().size(); iTrk++){
		if( ROOT::Math::VectorUtil::DeltaR( isoTracks_p4().at(iTrk), genleps_p4().at(iGen) ) < minDr_tau &&
		    isoTracks_isVetoTrack_v3().at(iTrk) ){
		  h_genHadTau_pt_isIsoTrack->Fill( genleps_p4().at(iGen).Pt(), wgt );
		  isMatchedToIsoTrack=true;
		  break;
		}
	      }
	    
	      if( isMatchedToVetoTau|| isMatchedToIsoTrack ){
		h_genHadTau_pt_isIsoTrackOrTauId->Fill( genleps_p4().at(iGen).Pt(), wgt );
	      }
	...
	if(isNominal){
	  ...
	  nEvents_pass_tauVeto_wgt+=wgt;
	  if(MT2W()<200.0 && nSelJets>=3){
	    nEvents_pass_tauVeto_lowDM++;
	    nEvents_pass_tauVeto_lowDM_wgt+=wgt;
	  }
	  if(MT2W()>=200.0 && nSelJets>=3){
	    nEvents_pass_tauVeto_highDM++;
	    nEvents_pass_tauVeto_highDM_wgt+=wgt;
	  }
	}
	...
	if( applyVetoLepSFs && !is_data() ){
	  if(appliedVetoLepSF){
	    if(histos.systematics[iSys]->id==k_lepSFUp) h_wgt_vetoLepSF_up->Fill( vetoLepSF_Up, scale1fb_reCalc*lumi );
	    else if(histos.systematics[iSys]->id==k_lepSFDown) h_wgt_vetoLepSF_dn->Fill( vetoLepSF_Dn, scale1fb_reCalc*lumi );
	    else{
	      h_wgt_vetoLepSF->Fill( vetoLepSF, scale1fb_reCalc*lumi );
	      if(abs(genSecondLep_pdgId)==11) h_wgt_vetoLepSF_el->Fill( vetoLepSF, scale1fb_reCalc*lumi );
	      if(abs(genSecondLep_pdgId)==13){
		h_wgt_vetoLepSF_mu->Fill( vetoLepSF, scale1fb_reCalc*lumi );
		h_wgt_vetoLepSF_mu_vs_pt->Fill( genSecondLep_TLV.Pt(), vetoLepSF, scale1fb_reCalc*lumi );
	      }
	    }
	  }
	}
	...
	if( nSelJets==2){
	  if( topnessMod()>=6.4 ){
	    ...
	    if( pfmet()>=250.0 && pfmet()<350.0){
	      for(int iCat=0; iCat<(int)histos.categories.size(); iCat++){
		if(k_compDM_ee2j_met_250_350 == histos.categories[iCat]->id) cat_idx.push_back(iCat);
	      }
	    } // end if 250<MET<350

	    if( pfmet()>=350.0 ){
	      for(int iCat=0; iCat<(int)histos.categories.size(); iCat++){
		if(k_compDM_ee2j_met_350_inf == histos.categories[iCat]->id) cat_idx.push_back(iCat);
	      }
	    } // end if MET>=350

	  } // end if topnessMod>=6.4
	...
	if( nSelJets==3){
	  ...
	  if( MT2W()>200.0 ){
	    ...
	    if( pfmet()>=250.0 && pfmet()<350.0){
	      for(int iCat=0; iCat<(int)histos.categories.size(); iCat++){
		if(k_highDM_ee3j_met_250_350 == histos.categories[iCat]->id) cat_idx.push_back(iCat);
	      }
	    } // end if 250<MET<350
	...
	for(int iClassy=0; iClassy<(int)classy_idx.size(); iClassy++){
	  for(int iCat=0; iCat<(int)cat_idx.size(); iCat++){
	    ...
	    if( applyVetoLepSFs && !is_data() ){
	      if(histos.systematics[iSys]->id==k_lepSFUp) histos.var_yield_only_vetoLepSF->histo_[hist_idx]->Fill( 1.0, scale1fb_reCalc*lumi*vetoLepSF_Up );
	      else if(histos.systematics[iSys]->id==k_lepSFDown) histos.var_yield_only_vetoLepSF->histo_[hist_idx]->Fill( 1.0, scale1fb_reCalc*lumi*vetoLepSF_Dn );
	      else histos.var_yield_only_vetoLepSF->histo_[hist_idx]->Fill( 1.0, scale1fb_reCalc*lumi*vetoLepSF );
	    }
  ...
  */
  ...
  cout << "    nEvents Pass >=4 Selected Jets (wgt) = " << nEvents_pass_nJets << " (" << nEvents_pass_nJets_wgt << "),  Eff = " << (double)nEvents_pass_nJets/(double)nEvents_pass_nVetoLep << endl;
  ...
  cout << "    nEvents Pass Track Veto (wgt)        = " << nEvents_pass_trackVeto << " (" << nEvents_pass_trackVeto_wgt << "),  Eff = " << (double)nEvents_pass_trackVeto/(double)nEvents_pass_minDPhi << endl;
  cout << "    nEvents Pass Tau Veto (wgt)          = " << nEvents_pass_tauVeto << " (" << nEvents_pass_tauVeto_wgt << "),  Eff = " << (double)nEvents_pass_tauVeto/(double)nEvents_pass_trackVeto << endl;
  cout << "            tau Veto Eff, low DM  = " << (double)nEvents_pass_tauVeto_lowDM/nEvents_lowDM << endl;
  cout << "            tau Veto Eff, high DM = " << (double)nEvents_pass_tauVeto_highDM/nEvents_highDM << endl;
  cout << endl;
